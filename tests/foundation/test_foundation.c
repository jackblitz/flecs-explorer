...
static int TestAssertionsPass(void) {
    ...
    TEST_ASSERT_STR_EQ("flecs_explorer", "flecs_explorer", "String equality should pass");
...
static int TestCJsonParseAndSerialize(void) {
    const char *jsonText = "{\"name\":\"Position\",\"x\":10.5,\"y\":20.0,\"active\":true}";
    ...
    TEST_ASSERT_STR_EQ(nameItem->valuestring, "Position", "'name' value should match");
